...
- `memory.enhancedmem.bm25MinScoreRatio`: BM25 相对阈值，仅保留 `score >= max_score * ratio` 的结果（默认 0.1），用于过滤与 query 不相关的内容。
- `memory.enhancedmem.bm25MinScoreAbsolute`: BM25 绝对阈值，若 `max_score < 此值` 则返回空（默认 0.01），表示 query 与语料无关。
